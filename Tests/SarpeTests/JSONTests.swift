...
final class SarpeTests: XCTestCase {
  ...
  func testString() throws {
    ...
    XCTAssertEqual(
      jsonString.parse("\"he\\rllo there \\u00ff\""), .success(JSON.string("he\rllo there ÿ"), ""))
    XCTAssertEqual(
      jsonString.parse("\"he\\rllo there \\u00ff\""),
      .success(JSON.string("he\rllo there \u{00ff}"), ""))
  ...
  func testArray() throws {
    ...
    XCTAssertEqual(
      jsonArray.parse("[[[]]]"), .success(JSON.array([JSON.array([JSON.array([])])]), ""))
    XCTAssertEqual(
      jsonArray.parse("[[5, [\"hei\"]]]"),
      .success(
        JSON.array([
          JSON.array([JSON.uint(5), JSON.array([JSON.string("hei")])])
        ]), ""))
    XCTAssertEqual(
      jsonArray.parse("[3 ,   \"fish\"]"),
      .success(JSON.array([JSON.uint(3), JSON.string("fish")]), ""))
  ...
  func testObject() throws {
    ...
    XCTAssertEqual(
      jsonObject.parse("{ \"fish\" :3}"), .success(JSON.object(["fish": JSON.uint(3)]), ""))
  ...
  func testIntegrationSmall() throws {
    let test1format1 = """
      ...
      """
    let test1format2 =
      #"[[{"using":"notice","solar":true,"trace":-1619953567,"another":"shorter","certain":false,"attack":"cool"},false,true, -2036083992,1706257801.7564554,"friend"],"may","bad",true,true,false]"#
    ...
    let test1Expected = Parse<Substring, JSON>.success(
      JSON.array([
        JSON.array([
          JSON.object([
            "using": JSON.string("notice"),
            "solar": JSON.bool(true),
            "trace": JSON.int(-1_619_953_567),
      ...
      .array([
        .object([
          "foot": .array([
            .object([
              "tone": .object([
                "ever": .int(-2_090_952_406),
                "top": .double(16053742.921692848),
                "middle": .object([
                  "move": .array([
                    .object([
                      "subject": .string("engineer"),
                      "bottle": .int(-1_586_440_403),
                      "hang": .array([
                        .uint(136_820_061),
                        .double(-837346463.2924082),
                        .object([
                          "wait": .object([
                            "level": .int(-293_146_016),
                            "depend": .int(-1_825_927_357),
                  ...
                  "gravity": .int(-334_952_669),
                  "mountain": .string("largest"),
                  "flow": .string("element"),
                ]),
                "lack": .uint(1_776_536_515),
              ]),
